...
vector<double> Spectogram::getFFTMagnitudes(vector<double> amplitudes) 
{
    
    int sampleSize = amplitudes.size();
    // call the fft and transform the complex numbers
    FFT fft = FFT(sampleSize / 2, -1);
    fft.transform(amplitudes);
    // end call the fft and transform the complex numbers

    vector<double> complexNumbers = amplitudes;
    ...
    vector<double> mag = new double[positiveSize];
    for (int i = 0; i < indexSize; i += 2) 
    {
        mag[i / 2] = sqrt(complexNumbers[i] * complexNumbers[i] + complexNumbers[i + 1] * complexNumbers[i + 1]);
    }
